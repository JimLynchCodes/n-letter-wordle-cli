...
fn get_score_style_for_guess_state(text: String, guess_state: &GuessState) -> String {
    match guess_state {
...
pub fn build_colored_guess_string(
    scored_letters: Vec<(String, GuessState)>,
) -> Result<String, Box<dyn Error>> {
    println!("\nScoring guess...\n");

    Ok(scored_letters
        .into_iter()
        .map(|(text, guess_state)| get_score_style_for_guess_state(text, &guess_state))
        .collect::<Vec<String>>()
        .join(""))
}
